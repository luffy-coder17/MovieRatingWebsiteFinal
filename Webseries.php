...
$page_title = "ReviewHub: Web Series";

require_once('includes/header.php');
require_once('includes/database.php');

// Capture the search term and selected genre from the form submission
$search_term = isset($_GET['webseries']) ? $conn->real_escape_string($_GET['webseries']) : '';
$selected_genre = isset($_GET['filter']) ? $conn->real_escape_string($_GET['filter']) : '';

// Construct the SQL query to filter web series based on the search term and selected genre
$query_str = "SELECT * FROM movies WHERE content_type = 'webseries'"; // Filter for web series
...
<div class="container wrapper">
    ...
    <h1 class="text-center">
        ...
        if (!empty($search_term) || ($selected_genre && $selected_genre != 'all')) {
            if ($num_results > 0) {
                echo "Search Results"; // Display "Search Results" if web series are found
            } else {
                echo "0 results found"; // Display "0 results found" if no web series match the criteria
            }
        } else {
            echo "Web Series"; // Default title for the web series page
        }
        ?>
    </h1>
        ...
        <div class="col-xs-4 col-xs-offset-8">
            <form action="searchmovies.php" method="get" class="form-inline search-form" role="form">
                <div class="input-group">
                    <label class="sr-only" for="webseriesSearch">Search Web Series</label>
                    <div class="input-group-addon"><i class="fa fa-search fa-lg"></i></div>
                    <input type="text" name="webseries" placeholder="Search" id="webseriesSearch" class="form-control" value="<?php echo htmlspecialchars($search_term); ?>"/>
                </div>
    ...
    <div class="webseries-list">
        <?php
        if ($num_results > 0) {
            // Display each web series in the result set
            $i = 0;
            ...
            // Display a message if no web series are found
            if (!empty($search_term) || ($selected_genre && $selected_genre != 'all')) {
                echo "<p>No web series found matching your criteria.</p>";
            } else {
                // Optionally display a message when no search has been made yet
                echo "<p>Use the search box and filters to find web series.</p>";
            }
        }
        ?>
    </div>
